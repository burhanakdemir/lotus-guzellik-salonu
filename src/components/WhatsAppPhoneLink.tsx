...
export function WhatsAppPhoneLink({
  phone,
  className = "",
  iconClassName = "h-6 w-6 shrink-0 text-[#25D366] drop-shadow-sm md:h-7 md:w-7",
  children,
}: WhatsAppPhoneLinkProps) {
  const href = whatsappUrl(phone);
  const label = children ?? formatPhoneDisplay(phone) ?? phone;
  ...
  return (
    <a
      ...
      aria-label={`WhatsApp ile yazın: ${phone}`}
    >
